...
class PSEngine {
     ...
     */
    async initializeBattle(players, options) {
        ...
        for (const line of lines) {
            ...
            switch (cmd) {
                case "move": {
                    ...
                    const attackerSide = attacker?.slice(0, 2);
     ...
     */
    async waitForRequests(timeoutMs = 1000) {
     ...
     */
    async submitTeamOrder(playerId, order) {
        const side = this.playerIdToSide.get(playerId);
        if (!side || !this.battle)
            return;
        ...
        // Clear the request
        if (side === "p1")
            this.battle.p1Request = null;
        else
            this.battle.p2Request = null;
        await this.waitForRequests();
    }
    /**
     * Submit a force switch choice
     */
    async forceSwitch(playerId, toIndex) {
        const side = this.playerIdToSide.get(playerId);
        if (!side || !this.battle) {
    ...
    syncStateFromPS() {
        ...
        const updateFromRequest = (request, playerIndex) => {
...
exports.PSEngine = PSEngine;
exports.default = PSEngine;
//# sourceMappingURL=ps-engine.js.map