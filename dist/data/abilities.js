...
exports.Abilities = {
    ...
    drizzle: {
        ...
        onSwitchIn: (pokemon, state, log) => {
            ...
            if (pokemon.item && state && state) {
                ...
                turns = require("../items").Items[pokemon.item]?.onWeatherDuration?.("rain", turns) ?? turns;
            }
            state.field.weather = { id: "rain", turnsLeft: turns };
    ...
    drought: {
        ...
        onSwitchIn: (pokemon, state, log) => {
            let turns = 5;
            if (pokemon.item && state && state) {
                turns = require("../items").Items[pokemon.item]?.onWeatherDuration?.("sun", turns) ?? turns;
            }
            state.field.weather = { id: "sun", turnsLeft: turns };
    ...
    sand_stream: {
        ...
        onSwitchIn: (pokemon, state, log) => {
            let turns = 5;
            if (pokemon.item && state && state) {
                turns = require("../items").Items[pokemon.item]?.onWeatherDuration?.("sandstorm", turns) ?? turns;
            }
            state.field.weather = { id: "sandstorm", turnsLeft: turns };
    ...
    snow_warning: {
        ...
        onSwitchIn: (pokemon, state, log) => {
            let turns = 5;
            if (pokemon.item && state && state) {
                turns = require("../items").Items[pokemon.item]?.onWeatherDuration?.("snow", turns) ?? turns;
            }
            state.field.weather = { id: "snow", turnsLeft: turns };
