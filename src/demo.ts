...
import { samplePlayers } from "./samples";
import { ExternalDexData, ExternalTeam, mapMatchToPlayers } from "./adapters/pokedex-adapter";

const engine = new Engine({ seed: 123, deterministicTies: true });
// Example: use the adapter with a tiny embedded dex + team data
const externalDex: ExternalDexData = {
	species: {
...
const team1: ExternalTeam = { playerId: "p1", name: "Player 1", party: [{ speciesId: "eevee", moves: ["quick-attack", "tackle"], level: 50 }] };
const team2: ExternalTeam = { playerId: "p2", name: "Player 2", party: [{ speciesId: "charmander", moves: ["ember", "tackle"], level: 50 }] };

const players = mapMatchToPlayers([team1, team2], externalDex);
