import { Move, Stats, TypeName } from "../../types";
import { mergeSpecies, SpeciesEntry } from "../pokedex-registry";

const norm = (s: string) => (s || "").toLowerCase().replace(/\s+/g, "").replace(/[_ ]/g, "").replace(/[^a-z0-9]/g, "");

function mapTypes(ts: any): TypeName[] {
  const arr = Array.isArray(ts) ? ts : ts ? [ts] : [];
  return arr.map((t: string) => (t.charAt(0).toUpperCase() + t.slice(1).toLowerCase())) as TypeName[];
}

function mapStats(bs: any): Stats {
  return {
    hp: bs?.hp ?? 50,
    atk: bs?.atk ?? 50,
    def: bs?.def ?? 50,
...
export function convertShowdownSpecies(raw: Record<string, any>) {
  ...
  for (const [k, v] of Object.entries(raw || {})) {
    ...
    out[id] = {
      ...
      abilities: Object.values(v?.abilities || {}).map((a: any) => norm(String(a))),
    };
  }
  mergeSpecies(out);
  return out;
...
export function convertShowdownMoves(raw: Record<string, any>): Record<string, Move> {
  ...
  for (const [k, v] of Object.entries(raw || {})) {
    ...
    const move: Move = {
      id,
      name: v?.name || k,
      type: (v?.type || "Normal") as TypeName,
    ...
    if (id === "recover") {
      move.onUse = ({ user, log, utils }) => {
        ...
        if (healed > 0) log(`${user.name} recovered health! (+${healed})`);
        else log(`${user.name}'s HP is full!`);
      };
    }
